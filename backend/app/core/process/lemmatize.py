...
def lemmatize_sentence_to_tokens(sentence_row: SentenceRow, *, version_id: str) -> list[LemmaTokenRow]:
    ...
    token_index_map = {token.i: index for index, token in enumerate(non_space_tokens)}

    result: list[LemmaTokenRow] = []
    for word_index, token in enumerate(non_space_tokens):
        head_index = token_index_map.get(token.head.i, word_index)
        result.append(
            {
                "id": str(uuid4()),
                ...
                "lemma_word": token.lemma_ if token.lemma_ else token.text,
