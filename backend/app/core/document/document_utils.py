...
def get_file_type(file_path: str) -> str:
    ...
    try:
        ...
        log_event(LOGGER, stage="OK", module_file=MODULE_FILE,
            function_name=function_name, extension=extension, result=file_type)
        return file_type
    
    except Exception as error:
        log_event(LOGGER, stage="ERROR", module_file=MODULE_FILE, function_name=function_name,
            file_path=file_path, error=str(error), exc_info=True)
        raise
...
def calculate_sha256(file_path: str) -> str:
    ...
    try:
        ...
        log_event(LOGGER, stage="OK", module_file=MODULE_FILE,
            function_name=function_name, bytes_count=len(data), result=digest)
        return digest
    except Exception as error:
        log_event(LOGGER, stage="ERROR", module_file=MODULE_FILE, function_name=function_name,
            file_path=file_path, error=str(error), exc_info=True)
...
def get_file_size(file_path: str) -> int:
    ...
    try:
        ...
        log_event(LOGGER, stage="OK", module_file=MODULE_FILE,
            function_name=function_name, result=file_size)
        return file_size
    except Exception as error:
        log_event(LOGGER, stage="ERROR", module_file=MODULE_FILE, function_name=function_name,
            file_path=file_path, error=str(error), exc_info=True)
...
def ensure_storage_dir() -> Path:
    ...
    try:
        ...
        log_event(LOGGER, stage="OK", module_file=MODULE_FILE,
            function_name=function_name, result=storage_dir.as_posix())
        return storage_dir
    except Exception as error:
...
def load_txt_by_path(text_path: str) -> str:
    function_name = "load_txt_by_path"
    log_event(LOGGER, stage="CALL", module_file=MODULE_FILE,
        function_name=function_name, text_path=text_path)
    ...
    try:
        ...
        log_event(LOGGER, stage="OK", module_file=MODULE_FILE,
            function_name=function_name, text_path=text_path, text_length=len(text))
        return text
